...
class ProcessAPI:
    ...
    def _process_class(self, node: ElementTree.Element, indent: str = "") -> str:
        ...
        s += "\n"
        s += '"""\n'
        s += "from ansys.ensight.core.session import Session\n"
        s += "from ansys.ensight.core.ensobj import ENSOBJ\n"
        s += "from ansys.ensight.core import ensobjlist\n"
        if superclass != "ENSOBJ":
            s += f"from ansys.ensight.core.{superclass.lower()} import {superclass}\n"
        s += "from typing import Any, Dict, List, Type, Union, Optional, Tuple, TYPE_CHECKING, cast\n"
        s += "\n"
        s += "if TYPE_CHECKING:\n"
        s += "    from ansys.ensight.core.ensight_api import " + ", ".join(list_of_imports_cured)
        s += "\n\n"
        ...
        with open(pathname, "w", encoding="utf-8") as fp:
            fp.write(s)
        return f"from ansys.ensight.core.{classname.lower()} import {classname}\n"
    ...
    def process(self, dirname: str, filename: str) -> None:
        ...
        s += '"""\n'
        s += "from ansys.ensight.core import Session\n"
        s += "from ansys.ensight.core.ensobj import ENSOBJ\n"
        s += "from ansys.ensight.core import ensobjlist\n"
        s += "ENSIMPORTS"
...
def generate_stub_api() -> None:
    ...
    sys.path.append("../src")
    from ansys.ensight.core._version import DEFAULT_ANSYS_VERSION, VERSION
